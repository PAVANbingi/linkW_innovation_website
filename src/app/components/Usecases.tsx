      ...
      <div className="container mx-auto px-6 lg:px-16 flex flex-col-reverse lg:flex-row items-center gap-16">
        ...
        <div className="lg:w-1/2 grid grid-cols-1 sm:grid-cols-2 gap-8">
          {useCases.map((useCase, index) => (
            <div
              key={index}
              className="group flex items-start gap-4 p-6 bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-500 hover:scale-105 hover:bg-opacity-90 backdrop-blur-sm hover:bg-yellow-700"
            >
              <div className="w-16 h-16 flex-shrink-0 relative">
                {/* Icon Animation */}
                <div className="absolute inset-0 bg-yellow-300 rounded-full opacity-0 blur-lg group-hover:opacity-100 group-hover:scale-125 transition-all duration-300"></div>
                <Image
                  src={useCase.icon}
                  alt={useCase.title}
                  width={64}
              ...
              <div className="flex flex-col justify-center">
                <p className="text-black text-sm lg:text-base font-medium leading-snug group-hover:text-white group-hover:translate-x-1 group-hover:transition-all group-hover:duration-300">
                  {useCase.title}
                </p>
              </div>
            </div>
